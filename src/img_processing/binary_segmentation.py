...
def mask_single_slice(image: ndarray,
    ...
    # noise_img = adaptiveThreshold(blur,
    #                               255,
    ...
    # bilateral = bilateralFilter(noise_img, 15, 75, 75)
    # bilateral = GaussianBlur(noise_img, (15, 15), 0)

    ret, th = threshold(blur, 0, 255, THRESH_OTSU)

    # # converting image to 8bit
    # noise_img = convert_to_8bit(image=noise_img)
    th = convert_to_8bit(image=th)
...
def generate_binary_mask(input_path: str,
    ...
    # getting image mask
    image = mask_single_slice(image=image,
